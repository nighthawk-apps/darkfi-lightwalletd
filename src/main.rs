...
"#;
    ...
    let rpc_client = Arc::new(DarkfidRpcClient::new(endpoint));
    ...
    // Create gRPC service with UnifOMR rate limit (S6)
    let service = LightWalletService::with_rate_limit(
        Arc::clone(&cache),
    ...
    info!(
        target: "lightwalletd",
        "Starting gRPC server on {grpc_addr} (omr_rate_limit={}/min)",
        config.omr_rate_limit_per_min
    );
    ...
    let server_handle = tokio::spawn(async move {
        ...
        builder
            .add_service(
                DarkFiLightWalletServer::new(service)
                    // UnifOMR detection keys are ~38MB class (Param2 n=1024 BFV);
                    // gRPC / MAX_DETECTION_KEY_BYTES allows up to 48MB per key
                    // (64MB total across detection_keys).
